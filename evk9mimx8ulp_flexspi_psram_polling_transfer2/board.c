...
void BOARD_SetTrdcGlobalConfig(void)
{
    ...
    if (releasedTrdc == true)
    {
        ...
        /* non secure state can access 0(PBridge0 slot 57 of T-MBC2, also need setup T-MRC0) for cortex-A35 */
        mbcBlockConfig.memoryAccessControlSelect = TRDC_MBC_ACCESS_CONTROL_POLICY_ALL_INDEX;
        mbcBlockConfig.nseEnable                 = true; /* non secure state can access the block for cortex-A35 */
        mbcBlockConfig.mbcIdx                    = 2U;   /* MBC2 */
        mbcBlockConfig.domainIdx                 = 7U;   /* MBC2_DOM7 */
...
/* Indexes for each command sequence */
#define PSRAM_CMD_LUT_SEQ_IDX_ENTER_QUAD_MODE  0
#define PSRAM_CMD_LUT_SEQ_IDX_RESET_ENABLE     1
#define PSRAM_CMD_LUT_SEQ_IDX_RESET            2

static status_t psram_reset(FLEXSPI_Type *base)
{
    flexspi_transfer_t flashXfer;
    status_t status;

    /* Step 1: Send Reset Enable Command */
    flashXfer.deviceAddress = 0x0U;
    flashXfer.port          = kFLEXSPI_PortA1;
    flashXfer.cmdType       = kFLEXSPI_Command;
    flashXfer.SeqNumber     = 1;
    flashXfer.seqIndex      = PSRAM_CMD_LUT_SEQ_IDX_RESET_ENABLE;  // Index for Reset Enable command
    ...
    /* Delay to ensure Reset Enable command is processed */
    for (uint32_t i = 2000000U; i > 0; i--)
    {
        __NOP();
    }
    ...
    flashXfer.seqIndex = PSRAM_CMD_LUT_SEQ_IDX_RESET;  // Index for Reset command

    status = FLEXSPI_TransferBlocking(base, &flashXfer);
    if (status == kStatus_Success)
    {
        /* Additional delay after Reset command */
        for (uint32_t i = 2000000U; i > 0; i--)
        {
            __NOP();
        }
...
status_t flexspi_psram_enter_quad_mode(FLEXSPI_Type *base)
{
    flexspi_transfer_t flashXfer;
    status_t status;

    /* Set up the transfer for entering Quad mode */
    flashXfer.deviceAddress = 0x0U;
    flashXfer.port          = kFLEXSPI_PortA1;
    flashXfer.cmdType       = kFLEXSPI_Command;
    flashXfer.SeqNumber     = 1;
    flashXfer.seqIndex      = PSRAM_CMD_LUT_SEQ_IDX_ENTER_QUAD_MODE;  // Quad mode command index

    /* Send the command */
    status = FLEXSPI_TransferBlocking(base, &flashXfer);

    /* Optional delay to ensure the command completes */
    if (status == kStatus_Success)
    {
        for (uint32_t i = 200000U; i > 0; i--)  // Small delay
        {
            __NOP();
        }
    }
...
status_t BOARD_InitPsRam(void)
{
    flexspi_device_config_t deviceconfig = {
        .flexspiRootClk       = 196000000, /* 392MHZ SPI serial clock, DDR serial clock 196M */
        .isSck2Enabled        = false,
    ...
    // uint32_t customLUT[64] = {
    //     /* Enter Quad Mode */
    //     [0] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_1PAD, 0x35, kFLEXSPI_Command_STOP, kFLEXSPI_1PAD, 0x00),

    //     /* Reset Enable */
    //     [4] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_1PAD, 0x66, kFLEXSPI_Command_STOP, kFLEXSPI_1PAD, 0x00),
        
    //     /* Reset */
    //     [8] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_1PAD, 0x99, kFLEXSPI_Command_STOP, kFLEXSPI_1PAD, 0x00),

    //     /* Read Data (Quad I/O, 0xEB) */
    //     [12] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_4PAD, 0xEB, kFLEXSPI_Command_RADDR_SDR, kFLEXSPI_4PAD, 0x18),
    //     [13] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_DUMMY_SDR, kFLEXSPI_4PAD, 0x06, kFLEXSPI_Command_READ_SDR, kFLEXSPI_4PAD, 0x04),

    //     /* Write Data (Quad I/O, 0x38) */
    //     [16] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_4PAD, 0x38, kFLEXSPI_Command_RADDR_SDR, kFLEXSPI_4PAD, 0x18),
    //     [17] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_WRITE_SDR, kFLEXSPI_4PAD, 0x06, kFLEXSPI_Command_STOP, kFLEXSPI_1PAD, 0x00)
    // };

    uint32_t customLUT[64] = {
    /* Read Data */
    [0] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_1PAD, 0xEB, kFLEXSPI_Command_RADDR_SDR, kFLEXSPI_4PAD, 0x18),  // Command 'EB' for Read Data, address with 4 data lanes
    [1] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_DUMMY_RWDS_DDR, kFLEXSPI_4PAD, 0x06, kFLEXSPI_Command_READ_DDR, kFLEXSPI_4PAD, 0x04),  // Dummy Read

    /* Write Data */
    [4] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_1PAD, 0xA0, kFLEXSPI_Command_RADDR_DDR, kFLEXSPI_4PAD, 0x18),  // Command 'A0' for Write Data, address with 4 data lanes
    [5] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_DUMMY_RWDS_DDR, kFLEXSPI_4PAD, 0x06, kFLEXSPI_Command_WRITE_DDR, kFLEXSPI_4PAD, 0x04),  // Dummy Write

    /* Read Register */
    [8] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_1PAD, 0x40, kFLEXSPI_Command_RADDR_SDR, kFLEXSPI_4PAD, 0x18),  // Command '40' for Read Register, address with 4 data lanes
    [9] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_DUMMY_RWDS_DDR, kFLEXSPI_4PAD, 0x06, kFLEXSPI_Command_READ_DDR, kFLEXSPI_4PAD, 0x04),  // Dummy Read for Register

    /* Write Register */
    [12] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_1PAD, 0xC0, kFLEXSPI_Command_RADDR_SDR, kFLEXSPI_4PAD, 0x18),  // Command 'C0' for Write Register, address with 4 data lanes
    [13] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_WRITE_DDR, kFLEXSPI_4PAD, 0x08, kFLEXSPI_Command_STOP, kFLEXSPI_1PAD, 0x00),  // Write Stop command

    /* Reset */
    [16] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_1PAD, 0xFF, kFLEXSPI_Command_DUMMY_SDR, kFLEXSPI_4PAD, 0x03),  // Command 'FF' for Reset, dummy with 4 data lanes
};
    ...
    /* Reset hyper ram. */
    status = psram_reset(BOARD_FLEXSPI_PSRAM);
    if (status != kStatus_Success)
    {
        return status;
    }

    status = flexspi_psram_enter_quad_mode(BOARD_FLEXSPI_PSRAM);
    if (status != kStatus_Success)
    {
        return status;
    }
