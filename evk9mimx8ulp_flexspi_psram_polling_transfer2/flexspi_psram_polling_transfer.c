...
#define PSRAM_CMD_LUT_SEQ_IDX_READDATA         3
#define PSRAM_CMD_LUT_SEQ_IDX_WRITEDATA        4
...
status_t flexspi_psram_ipcommand_write_data(FLEXSPI_Type *base, uint32_t address, uint32_t *buffer, uint32_t length)
{
    flexspi_transfer_t flashXfer;
    status_t status;
    ...
    flashXfer.cmdType       = kFLEXSPI_Write;
    flashXfer.SeqNumber     = 1;
    flashXfer.seqIndex      = PSRAM_CMD_LUT_SEQ_IDX_WRITEDATA;  // Updated index for PSRAM write command
    flashXfer.data          = buffer;
    flashXfer.dataSize      = length;
...
status_t flexspi_psram_ipcommand_read_data(FLEXSPI_Type *base, uint32_t address, uint32_t *buffer, uint32_t length)
{
    flexspi_transfer_t flashXfer;
    status_t status;
    ...
    flashXfer.cmdType       = kFLEXSPI_Read;
    flashXfer.SeqNumber     = 1;
    flashXfer.seqIndex      = PSRAM_CMD_LUT_SEQ_IDX_READDATA;  // Updated index for PSRAM read command
    flashXfer.data          = buffer;
    flashXfer.dataSize      = length;
...
int main(void)
{
    ...
    {
        st = flexspi_psram_ipcommand_write_data(EXAMPLE_FLEXSPI, i, (uint32_t *)s_hyper_ram_write_buffer,
        ...
        st = flexspi_psram_ipcommand_read_data(EXAMPLE_FLEXSPI, i, (uint32_t *)s_hyper_ram_read_buffer,
